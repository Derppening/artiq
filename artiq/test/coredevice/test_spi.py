...
from artiq.language.core import kernel, print_rpc
...
class CardTest(EnvExperiment):
    ...
    def run(self):
        ...
        for i in range(10):
            self.spi_mmc.write(int32(uint32(0xffffffff)))
        ...
        self.spi_mmc.write(int32(uint32(0x40 << 24)))  # CMD
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG, 32, freq, cs)
        self.spi_mmc.write(int32(uint32(0x00000000)))  # ARG
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG, 8, freq, cs)
        self.spi_mmc.write(int32(uint32(0x95 << 24)))  # CRC
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG | SPI_INPUT, 8, freq, cs)
        idle = False
        response = 0
        for i in range(8):
            self.spi_mmc.write(int32(uint32(0xff << 24)))  # NCR
        ...
        self.spi_mmc.write(int32(uint32(0xff << 24)))
        if not idle:
            print_rpc(response)
